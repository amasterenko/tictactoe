...
/**
 * Class represents the game initialization for human-human game.
 *
 * @author AndrewMs
 * @version 1.0
 */
public class ConsoleGameInitHH implements ConsoleGameInit {
    ...
    public ConsoleGameInitHH(Scanner in, PrintWriter out) {
        this.in = in;
        this.out = out;
        this.gameType = GameType.HUMAN_HUMAN;
        ...
        this.player2 = new ConsolePlayer("player2", inputInt, out);
    }
    ...
    public boolean init(int boardMinSize, int boardMaxSize) {
        try {
            ...
            int firstTurnPrior = inputInt.ask("Will you start first or second? (1/2):",
                    p -> p == 1 || p == 2).get();
            int markType = inputInt.ask("Select your mark type (X - 1, O - 2):",
                    p -> p == 1 || p == 2).get();
            String name2 = inputStr.ask("Input the player's name:", str -> !str.isEmpty()).get();
            if (firstTurnPrior == 1) {
                player1 = new ConsolePlayer(name1, inputInt, out);
                player2 = new ConsolePlayer(name2, inputInt, out);
                mark.setX(markType == 1 ? 1 : 2);
                mark.setO(markType == 2 ? 1 : 2);
            }
            if (firstTurnPrior == 2) {
                player1 = new ConsolePlayer(name2, inputInt, out);
